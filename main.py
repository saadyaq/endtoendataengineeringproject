...
from zipfile import structEndArchive
...
def extract_start_date(file_content):
    try:
        opendate_match = re.search(r'(Open [Dd]ate:)\s{2}(\d\d-\d\d-\d\d)', file_content)
...
def extract_requirements(file_content):
    try:
        requirements_match=re.search(r'(REQUIREMENTS?/\s?MINIMUM QUALIFICATION?)(.*)(PROCESS NOTES?)', file_content,re.DOTALL)
...
def extract_experience_length(file_content):
    try:
        experience_length_match=re.search(
            r'(One|Two|Three|Four|Five|Six|Seven|Eight|Nine|one|two|three|four|five)\s(years?)\s(of\sfull(-|\s)time)',file_content
        )
...
if __name__=="__main__":
    ...
    csv_input_path=r"Unstructuredataproject/input/input_csv"
    img_input_path=r"Unstructuredataproject/input/input_img"
    json_input_path="input/input_json"
    pdf_input_path=r"Unstructuredataproject/input/input_pdf"
    text_input_path="input/input_text"
    video_input_path=r"Unstructuredataproject/input/input_video"
    ...
    job_bulletins_df = (spark.readStream
                        ...
                        .load(text_input_path)
                        )
    json_df=spark.readStream.json(json_input_path,schema=data_schema,multiLine=True)
    ...
    query=StreamWriter(union_dataframe,'s3a://s3-bucket-unstructuredyaq/checkpoints/',
                         's3a://s3-bucket-unstructuredyaq/data/spark_unstructured')
